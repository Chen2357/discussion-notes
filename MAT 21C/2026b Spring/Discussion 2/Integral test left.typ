...
#cetz.canvas(length: 1cm, {
  ...
  let left-rect(i) = {
    ...
    if i == 0 {
      content((x - dx / 2, f(N + i) / 2), anchor: "center")[#text(white, 7pt)[$a_N$]]
    } else {
      content((x - dx / 2, f(N + i) / 2), anchor: "center")[#text(white, 5pt)[$a_(N + #i)$]]
    }
  }
  ...
  content((x-start + (term-num - 0.5) * dx, f(N + term-num - 1) / 2), anchor: "center")[#text(blue.darken(20%))[$dots.c$]]

  let fun-points = nt.linspace(N, N + term-num, fun-num).map(n => (x-start + (n - N) * dx, f(n)))
  ...
  content((3, 3))[
    $display(sum_(n = N)^(oo) a_n <= a_N + integral_N^oo f(x) space.thin d x)$
  ]
})
